    ...
    <div style={{ background: '#ffffff', borderRadius: 18, boxShadow: '0 24px 64px rgba(13,22,50,0.22)', border: `1.5px solid ${RED}22`, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ background: `linear-gradient(135deg, ${NAVY_DARK} 0%, ${NAVY} 100%)`, padding: '18px 22px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
          ...
          <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.15em', textTransform: 'uppercase', color: 'rgba(255,255,255,0.7)', fontFamily: 'Inter, system-ui, sans-serif' }}>Urgent triage</div>
        </div>
        <div style={{ fontSize: 14, fontWeight: 700, color: '#ffffff', fontFamily: 'Inter, system-ui, sans-serif' }}>"What happened?" — get your next step now</div>
      </div>

      <div style={{ padding: '20px 22px 4px', flex: 1 }}>
        {history.length > 0 && (
          <button onClick={back} style={{ display: 'flex', alignItems: 'center', gap: 5, background: 'none', border: 'none', cursor: 'pointer', fontSize: 11.5, color: '#9ca3af', fontFamily: 'Inter, system-ui, sans-serif', padding: 0, marginBottom: 12 }}>
        ...
        {!isResult && q && (
          <>
            <p style={{ fontSize: 14, fontWeight: 600, color: NAVY_DARK, lineHeight: 1.45, margin: '0 0 14px', fontFamily: 'Inter, system-ui, sans-serif' }}>{q.q}</p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {q.opts.map(opt => (
                <button key={opt.label} onClick={() => choose(opt.next)}
                  style={{ padding: '11px 14px', background: 'rgba(220,38,38,0.08)', border: `1.5px solid ${RED}22`, borderRadius: 10, fontSize: 13, fontWeight: 500, color: NAVY_DARK, cursor: 'pointer', textAlign: 'left', fontFamily: 'Inter, system-ui, sans-serif', transition: 'all 0.15s' }}
          ...
          <div>
            {result.deadlineUrgent && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px', background: RED_LIGHT, border: `1.5px solid ${RED}`, borderRadius: 10, marginBottom: 14 }}>
                ...
                <div>
                  <div style={{ fontSize: 10, fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.1em', color: RED_DARK, fontFamily: 'Inter, system-ui, sans-serif' }}>Deadline</div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: RED_DARK, fontFamily: 'Inter, system-ui, sans-serif' }}>{result.deadline}</div>
                </div>
              </div>
            )}
            ...
            <div style={{ padding: '14px 16px', borderRadius: 12, border: `1.5px solid ${result.color}22`, background: `${result.color}06`, marginBottom: 12 }}>
              <div style={{ fontSize: 10, fontWeight: 800, letterSpacing: '0.12em', textTransform: 'uppercase', color: result.color, marginBottom: 6, fontFamily: 'Inter, system-ui, sans-serif' }}>Next step</div>
              <p style={{ fontSize: 13, color: '#374151', lineHeight: 1.62, margin: 0, fontFamily: 'Inter, system-ui, sans-serif' }}>{result.action}</p>
              {result.route && (
                <button onClick={() => navigate(result.route!)}
                  style={{ marginTop: 10, background: 'none', border: 'none', cursor: 'pointer', fontSize: 12.5, fontWeight: 700, color: result.color, fontFamily: 'Inter, system-ui, sans-serif', padding: 0, display: 'flex', alignItems: 'center', gap: 5 }}>
                  {result.routeLabel}
            ...
            <div style={{ fontSize: 11.5, color: '#6b7280', lineHeight: 1.55, fontFamily: 'Inter, system-ui, sans-serif', fontStyle: 'italic', marginBottom: 14 }}>{result.note}</div>

            <div style={{ display: 'flex', gap: 8 }}>
              <a href="#contact" style={{ flex: 1, padding: '12px 14px', background: RED, color: '#ffffff', border: 'none', borderRadius: 9, fontSize: 13, fontWeight: 700, cursor: 'pointer', textAlign: 'center', textDecoration: 'none', fontFamily: 'Inter, system-ui, sans-serif' }}>
                Request same-day triage →
              </a>
              <button onClick={restart} style={{ padding: '12px 14px', background: '#f3f4f8', color: '#6b7280', border: 'none', borderRadius: 9, fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'Inter, system-ui, sans-serif' }}>
                Start over
              </button>
            </div>
          </div>
      ...
      <div style={{ padding: '14px 22px', fontSize: 10.5, color: '#9ca3af', fontFamily: 'Inter, system-ui, sans-serif', fontStyle: 'italic' }}>
        ...
        <div style={{ maxWidth: 1160, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 420px', gap: 56, alignItems: 'start' }} className="hero-grid">
          <div>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8, marginBottom: 24, padding: '5px 14px 5px 6px', background: RED_LIGHT, border: `1.5px solid ${RED}55`, borderRadius: 100 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, color: RED_DARK, letterSpacing: '0.08em', textTransform: 'uppercase', fontFamily: 'Inter, system-ui, sans-serif' }}>Time-critical — deadlines are strict</span>
            </div>

            <h2 style={{ fontFamily: "'Fraunces', Georgia, serif", fontSize: 'clamp(28px, 3.5vw, 44px)', fontWeight: 700, color: NAVY, margin: '0 0 16px', letterSpacing: '-0.02em' }}>
              Use the triage tool to find your next step
            </h2>

            <p style={{ fontSize: 16, color: '#4b5563', lineHeight: 1.68, margin: '0 0 24px', fontFamily: 'Inter, system-ui, sans-serif' }}>
            ...
            <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <a href="#contact"
                style={{ background: RED, color: '#ffffff', padding: '14px 30px', borderRadius: 8, textDecoration: 'none', fontSize: 14, fontWeight: 700, display: 'inline-flex', alignItems: 'center', gap: 8, boxShadow: `0 4px 20px ${RED}44`, fontFamily: 'Inter, system-ui, sans-serif' }}>
                Request same-day triage →
              </a>
              <button onClick={() => navigate('art-review')}
                style={{ background: '#ffffff', color: NAVY, padding: '14px 30px', borderRadius: 8, fontSize: 14, fontWeight: 700, fontFamily: 'Inter, system-ui, sans-serif', border: `1.5px solid ${NAVY}22`, cursor: 'pointer', boxShadow: '0 2px 8px rgba(27,43,94,0.08)' }}>
                ART Review guide →
              </button>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 0 }} className="grid-4">
            {[
              ...
              <div key={i} style={{ padding: '16px 24px', borderRight: i < 3 ? '1px solid rgba(27,43,94,0.1)' : 'none' }}>
                <div style={{ fontFamily: "'Fraunces', Georgia, serif", fontSize: 'clamp(20px, 2.2vw, 30px)', fontWeight: 800, color: i === 3 ? RED : NAVY, lineHeight: 1 }}>{s.val}</div>
                <div style={{ fontSize: 12, color: '#6b7280', marginTop: 4, fontFamily: 'Inter, system-ui, sans-serif', lineHeight: 1.4 }}>{s.label}<span style={{ fontSize: 10, display: 'block', color: '#9ca3af' }}>{s.note}</span></div>
              </div>
            ))}
          </div>
          <p style={{ fontSize: 10.5, color: '#9ca3af', fontStyle: 'italic', margin: '10px 0 0', fontFamily: 'Inter, system-ui, sans-serif' }}>{DISCLAIMER}</p>
        </div>
      </section>
      ...
      <section style={{ background: GREY_BAND, padding: '88px 24px', borderTop: '1px solid #e8edf5' }}>
        <div style={{ maxWidth: 1160, margin: '0 auto' }}>
            ...
            {CARDS.map(card => {
              ...
              return (
                ...
                >
                  ...
                  <p style={{ fontSize: 13, color: '#6b7280', lineHeight: 1.65, margin: 0, fontFamily: 'Inter, system-ui, sans-serif', flex: 1 }}>{card.body}</p>
                  {clickable && (
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 20 }}>
            ...
            <span style={{ fontSize: 12, color: '#6b7280', fontFamily: 'Inter, system-ui, sans-serif' }}>Indicates a time-critical item — deadlines apply</span>
          </div>
        </div>
      </section>
        ...
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '400px 1fr', gap: 56, alignItems: 'start' }} className="hero-grid">
            <div>
              ...
              <p style={{ fontSize: 14, color: '#4b5563', lineHeight: 1.75, marginBottom: 16, fontFamily: 'Inter, system-ui, sans-serif' }}>
              ...
              <p style={{ fontSize: 14, color: '#4b5563', lineHeight: 1.75, marginBottom: 20, fontFamily: 'Inter, system-ui, sans-serif' }}>
              ...
              <div style={{ background: `${GOLD}14`, border: `1.5px solid ${GOLD}44`, borderRadius: 12, padding: '14px 16px' }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: '#0d1632', marginBottom: 4, fontFamily: 'Inter, system-ui, sans-serif' }}>ART Processing Time 2025–26</div>
                <div style={{ fontSize: 13, color: '#0d1632', lineHeight: 1.6, fontFamily: 'Inter, system-ui, sans-serif' }}>
              ...
              {[
                ...
                <div key={step.num} style={{ display: 'flex', gap: 20, position: 'relative', paddingBottom: i < 3 ? 32 : 0 }}>
                  ...
                  <div style={{ width: 40, height: 40, borderRadius: '50%', background: NAVY, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, zIndex: 1 }}>
                    <span style={{ fontFamily: "'Fraunces', Georgia, serif", fontSize: 13, fontWeight: 800, color: '#fff' }}>{step.num}</span>
                  </div>
                  <div style={{ paddingTop: 6, flex: 1 }}>
                    <div style={{ fontSize: 15, fontWeight: 700, color: NAVY, marginBottom: 6, fontFamily: 'Inter, system-ui, sans-serif' }}>{step.title}</div>
                    <div style={{ fontSize: 13, color: '#6b7280', lineHeight: 1.65, fontFamily: 'Inter, system-ui, sans-serif' }}>{step.desc}</div>
                  </div>
                </div>
              ))}
            </div>
            ...
            {[
              ...
              <div key={bar.title} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 18, padding: '24px 24px', backdropFilter: 'blur(12px)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                  <div style={{ fontSize: 15, fontWeight: 700, color: '#fff', lineHeight: 1.25, fontFamily: 'Inter, system-ui, sans-serif' }}>{bar.title}</div>
                  <span style={{ fontSize: 9, fontWeight: 800, letterSpacing: '0.08em', textTransform: 'uppercase', borderRadius: 100, padding: '3px 9px', flexShrink: 0, color: bar.impactColor, background: `${bar.impactColor}22`, border: `1px solid ${bar.impactColor}44`, fontFamily: 'Inter, system-ui, sans-serif', marginLeft: 10 }}>
                    {bar.impact}
                  </span>
                </div>
                <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.65)', lineHeight: 1.7, margin: '0 0 12px', fontFamily: 'Inter, system-ui, sans-serif' }}>{bar.desc}</p>
                <div style={{ display: 'inline-flex', alignItems: 'center', gap: 5, background: `${RED}18`, border: `1px solid ${RED}30`, borderRadius: 100, padding: '3px 10px' }}>
                  ...
                  <span style={{ fontSize: 10, fontWeight: 700, color: 'rgba(220,38,38,0.4)', letterSpacing: '0.06em', textTransform: 'uppercase', fontFamily: 'Inter, system-ui, sans-serif' }}>Time limits apply</span>
                </div>
              </div>
            ))}
          </div>
          <p style={{ fontSize: 10.5, color: 'rgba(255,255,255,0.35)', textAlign: 'center', marginTop: 20, fontFamily: 'Inter, system-ui, sans-serif' }}>{DISCLAIMER}</p>
        </div>
      </section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '400px 1fr', gap: 56, alignItems: 'start' }} className="hero-grid">
            <div>
              ...
              <div style={{ background: 'rgba(220,38,38,0.08)', border: `1.5px solid ${RED}44`, borderRadius: 12, padding: '14px 16px' }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: RED, marginBottom: 4, fontFamily: 'Inter, system-ui, sans-serif' }}>Response deadlines are not extended.</div>
                <div style={{ fontSize: 12, color: '#0d1632', lineHeight: 1.6, fontFamily: 'Inter, system-ui, sans-serif' }}>Contact us the day you receive the letter.</div>
              </div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ background: '#ffffff', borderRadius: 16, border: '1.5px solid rgba(245,161,36,0.12)', padding: '22px 22px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                  ...
                  <div style={{ fontSize: 12, fontWeight: 800, color: '#0d1632', letterSpacing: '0.06em', textTransform: 'uppercase', fontFamily: 'Inter, system-ui, sans-serif' }}>Do</div>
                </div>
                {['Respond within the timeframe stated in the letter', 'Address each specific adverse finding individually', 'Provide supporting documents and statutory declarations', 'Have a registered agent review your response before submission'].map(item => (
                  <div key={item} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, marginBottom: 9 }}>
                    ...
                    <span style={{ fontSize: 13, color: '#374151', lineHeight: 1.6, fontFamily: 'Inter, system-ui, sans-serif' }}>{item}</span>
                  </div>
                ))}
              </div>
              ...
              <div style={{ background: '#ffffff', borderRadius: 16, border: `1.5px solid ${RED}44`, padding: '22px 22px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                  ...
                  <div style={{ fontSize: 12, fontWeight: 800, color: RED, letterSpacing: '0.06em', textTransform: 'uppercase', fontFamily: 'Inter, system-ui, sans-serif' }}>{"Don't"}</div>
                </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 20 }} className="grid-3">
            {[
              ...
              <div key={issue.title} style={{ background: GREY_BAND, borderRadius: 16, border: '1.5px solid #e5eaf4', padding: '26px 24px' }}>
                ...
                <h3 style={{ fontFamily: "'Fraunces', Georgia, serif", fontSize: 17, fontWeight: 700, color: NAVY, margin: '0 0 10px', lineHeight: 1.3 }}>{issue.title}</h3>
                <p style={{ fontSize: 13, color: '#6b7280', lineHeight: 1.7, margin: 0, fontFamily: 'Inter, system-ui, sans-serif' }}>{issue.desc}</p>
              </div>
            ))}
          </div>
          <p style={{ fontSize: 10.5, color: '#9ca3af', textAlign: 'center', marginTop: 20, fontFamily: 'Inter, system-ui, sans-serif' }}>{DISCLAIMER}</p>
        </div>
      </section>
      ...
      <section style={{ background: `linear-gradient(160deg, ${NAVY_DARK} 0%, ${NAVY} 55%, ${NAVY_MID} 100%)`, padding: '88px 24px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', position: 'relative' }}>
          <div style={{ marginBottom: 40 }}>
            <div style={{ fontSize: 11, fontWeight: 800, letterSpacing: '0.16em', textTransform: 'uppercase', color: GOLD, marginBottom: 12, fontFamily: 'Inter, system-ui, sans-serif' }}>Act immediately</div>
            <h2 style={{ fontFamily: "'Fraunces', Georgia, serif", fontSize: 'clamp(26px, 3vw, 40px)', fontWeight: 700, color: '#ffffff', margin: 0, letterSpacing: '-0.02em', lineHeight: 1.15 }}>
              ...
              <div key={step.num} style={{ display: 'grid', gridTemplateColumns: '72px 1fr', gap: 0 }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'rgba(255,255,255,0.15)', border: '2px solid rgba(255,255,255,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, zIndex: 1 }}>
                    <span style={{ fontFamily: "'Fraunces', Georgia, serif", fontSize: 17, fontWeight: 800, color: '#ffffff' }}>{step.num}</span>
                  </div>
                ...
                <div style={{ paddingBottom: i < URGENT_STEPS.length - 1 ? 36 : 0, paddingLeft: 16 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6, flexWrap: 'wrap' }}>
                    ...
                    <span style={{ fontSize: 10, fontWeight: 700, padding: '3px 9px', borderRadius: 100, background: 'rgba(255,255,255,0.15)', color: GOLD, fontFamily: 'Inter, system-ui, sans-serif', whiteSpace: 'nowrap', flexShrink: 0 }}>{step.timing}</span>
                  </div>
                  <p style={{ fontSize: 13.5, color: 'rgba(255,255,255,0.72)', lineHeight: 1.7, margin: 0, fontFamily: 'Inter, system-ui, sans-serif' }}>{step.body}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div style={{ marginTop: 44, display: 'flex', justifyContent: 'center' }}>
            <a href="#contact"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 10, background: '#ffffff', color: NAVY_DARK, padding: '16px 40px', borderRadius: 9, fontSize: 15, fontWeight: 700, textDecoration: 'none', boxShadow: '0 6px 28px rgba(0,0,0,0.2)', fontFamily: 'Inter, system-ui, sans-serif' }}>
              Request same-day triage
      ...
      <section style={{ background: '#ffffff', padding: '88px 24px', borderTop: '1px solid #e8edf5' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
            ...
            {REVIEW_RIGHTS.map((row, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.8fr 1fr 1fr 1fr', padding: '13px 20px', background: i % 2 === 0 ? '#ffffff' : '#fafbfe', borderTop: '1px solid #f0f2f7', gap: 12, alignItems: 'center' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <span style={{ fontSize: 13, color: '#374151', fontFamily: 'Inter, system-ui, sans-serif', fontWeight: 500 }}>{row.type}</span>
                </div>
                <div style={{ fontSize: 12.5, color: row.art.startsWith('Yes') ? GREEN : row.art === 'No' || row.art === 'Generally no' ? '#dc2626' : AMBER, fontWeight: 600, fontFamily: 'Inter, system-ui, sans-serif' }}>{row.art}</div>
                <div style={{ fontSize: 12.5, color: '#6b7280', fontFamily: 'Inter, system-ui, sans-serif' }}>{row.federal}</div>
                <div style={{ fontSize: 12.5, color: '#6b7280', fontFamily: 'Inter, system-ui, sans-serif' }}>{row.ministerial}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ marginTop: 16, padding: '14px 18px', background: `${RED}08`, border: `1px solid ${RED}22`, borderRadius: 10, display: 'flex', alignItems: 'flex-start', gap: 10 }}>
            <Icon name="alert" size={14} color={RED} />
            <p style={{ fontSize: 12, color: RED_DARK, lineHeight: 1.6, margin: 0, fontFamily: 'Inter, system-ui, sans-serif' }}>
