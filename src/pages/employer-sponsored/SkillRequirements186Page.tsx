    ...
    <section id="work-experience" style={{ background: GREY_BG, padding: '80px 32px' }}>
      <div style={{ maxWidth: 1060, margin: '0 auto' }}>
        ...
        <div className="grid-2" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28, marginBottom: 28 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 10 }}>
              {WE_COUNTS.map((item, i) => (
                <div key={i} style={{ display: 'flex', gap: 10, alignItems: 'flex-start' }}>
                  ...
                  <span style={{ fontSize: 14, color: '#374151', lineHeight: 1.55 }}>{item.label}</span>
                </div>
              ))}
            </div>
          </div>
          {/* Evidence needed */}
          <div style={{ background: '#fff', border: `1px solid ${BORDER}`, borderRadius: 14, padding: '28px 26px', boxShadow: '0 2px 12px rgba(27,43,94,0.05)' }}>
            <h3 style={{ fontFamily: "'Gilroy', sans-serif", fontSize: 19, fontWeight: 700, color: NAVY, margin: '0 0 20px' }}>Evidence generally required</h3>
            <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 14 }}>
              {WE_EVIDENCE.map((item, i) => (
                <div key={i} style={{ display: 'flex', gap: 12, alignItems: 'flex-start', padding: '12px 14px', background: GREY_BG, borderRadius: 10, border: `1px solid ${BORDER}` }}>
                  ...
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: NAVY, marginBottom: 3 }}>{item.label}</div>
                    <div style={{ fontSize: 13, color: '#6b7280', lineHeight: 1.55 }}>{item.note}</div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div style={{ background: `${NAVY}05`, border: `1.5px solid ${NAVY}18`, borderRadius: 12, padding: '18px 22px', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
          ...
          <div style={{ fontSize: 14.5, color: '#374151', lineHeight: 1.7 }}>
    ...
    <div style={{ fontFamily: "'Gilroy', sans-serif", backgroundColor: '#ffffff', color: NAVY }}>
      ...
      <section id="evidence" style={{ background: GREY_BG, padding: '80px 32px' }}>
        <div style={{ maxWidth: 860, margin: '0 auto' }}>
          ...
          <p style={{ textAlign: 'center', fontSize: 16, color: '#6b7280', margin: '-24px auto 40px', maxWidth: 560 }}>An indicative checklist — your specific case may require additional documents. Always confirm requirements with your registered migration agent.</p>
