          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: 8, marginBottom: 40, flexWrap: 'wrap' }}>
            {([['partner', 'Partner Visas', ROSE], ['parent', 'Parent Visas', PURPLE], ['other', 'Child & Other', GREEN]] as const).map(([key, label, color]) => (
              <button key={key} onClick={() => setActiveTab(key)}
                style={{ padding: '10px 24px', borderRadius: 100, border: `2px solid ${activeTab === key ? color : '#e2e8f0'}`, background: activeTab === key ? color : '#ffffff', color: activeTab === key ? '#ffffff' : '#6b7280', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: "'Gilroy', sans-serif", transition: 'all 0.2s' }}>
                {label}
              </button>
            ))}
          </div>
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 8, marginBottom: 14 }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 9px', borderRadius: 100, background: `${card.tagColor}14`, color: card.tagColor, border: `1px solid ${card.tagColor}28`, fontFamily: "'Gilroy', sans-serif", textAlign: 'right' }}>{card.tag}</span>
                  </div>
                  <h3 style={{ fontFamily: "'Gilroy', sans-serif", fontSize: 19, fontWeight: 700, color: NAVY, margin: '0 0 10px', lineHeight: 1.2 }}>{card.name}</h3>
                  <p style={{ fontSize: 14, color: '#6b7280', lineHeight: 1.65, margin: '0 0 14px', fontFamily: "'Gilroy', sans-serif", flex: 1 }}>{card.body}</p>
                  {'note' in card && card.note && (
                    <div style={{ fontSize: 11.5, fontWeight: 600, color: card.tagColor, fontFamily: "'Gilroy', sans-serif", borderTop: `1px solid ${card.tagColor}1a`, paddingTop: 10, marginTop: 'auto' }}>{card.note}</div>
                  )}
                  {clickable && (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 20 }} className="grid-4">
            {PILLARS.map(p => (
              <div key={p.title} style={{ background: '#f8fafd', borderTop: `3px solid ${p.color}`, borderRadius: 16, padding: '28px 22px', border: `1px solid ${p.color}1a`, borderTopWidth: 3 }}>
                ...
                <h3 style={{ fontFamily: "'Gilroy', sans-serif", fontSize: 21, fontWeight: 700, color: NAVY, margin: '0 0 14px' }}>{p.title}</h3>
                <ul style={{ margin: 0, padding: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 7 }}>
                  {p.items.map(item => (
                    <li key={item} style={{ display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                      <Icon name="check" size={11} color={p.color} />
                      <span style={{ fontSize: 13.5, color: '#4b5563', fontFamily: "'Gilroy', sans-serif", lineHeight: 1.5 }}>{item}</span>
                    </li>
                  ))}
                </ul>
                <div style={{ marginTop: 16, padding: '10px 12px', background: `${p.color}08`, borderRadius: 8, fontSize: 12, color: p.color, fontWeight: 600, fontFamily: "'Gilroy', sans-serif", lineHeight: 1.45 }}>{p.note}</div>
              </div>
            ))}
          </div>
        </div>
