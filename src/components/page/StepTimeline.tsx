...
export function StepTimeline({ steps, variant = 'cards', accent = NAVY }: StepTimelineProps) {
  ...
  if (variant === 'flow') {
    // 820-style: compact, centered, 6-up
    const cols = Array.from({ length: n + dividers }, (_, i) => (i % 2 === 0 ? '1fr' : '40px')).join(' ')
    return (
      <div className="step-timeline" style={{ display: 'grid', gridTemplateColumns: cols, gap: 0, alignItems: 'center', maxWidth: 1100, margin: '0 auto' }}>
        {steps.map((stage, i) => {
  ...
  return (
    <div className="step-timeline" style={{ display: 'grid', gridTemplateColumns: cardCols, gap: 0, alignItems: 'center', maxWidth: 1000, margin: '0 auto' }}>
      {steps.map((stage, i) => {
        const color = stage.color ?? accent
        return (
          <React.Fragment key={i}>
