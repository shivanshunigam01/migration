      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, gap: 8 }}>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            <label htmlFor={`pts-${cat.id}`} style={{ fontSize: 13, fontWeight: 600, color: NAVY, lineHeight: 1.3, cursor: 'pointer' }}>
              {cat.heading}
            </label>
          </div>
          {isActive && (
            <div style={{ fontSize: 13, fontWeight: 800, color: cat.color, flexShrink: 0, minWidth: 48, textAlign: 'right' }}>
              +{pts} pts
            </div>
          )}
          {!isActive && (
            <div style={{ fontSize: 11, color: '#9ca3af', flexShrink: 0 }}>max {cat.maxPoints} pts</div>
          )}
        </div>
        <select
          id={`pts-${cat.id}`}
        ...
        {(() => {
          ...
          return selectedOpt?.note ? (
            <p style={{ margin: '7px 0 0', fontSize: 11.5, color: '#6b7280', lineHeight: 1.5 }}>ℹ️ {selectedOpt.note}</p>
          ) : null
        })()}
      </div>
    )
    ...
    <div className="points-result-card" style={{ position: isMobile ? 'static' : 'sticky', top: 80, width: isMobile ? '100%' : 300, marginTop: isMobile ? 24 : 0 }}>
      ...
      <div style={{ background: '#fff', border: `2px solid ${total >= POINTS_MINIMUM ? accent : BORDER}`, borderRadius: 16, padding: '24px 22px', marginBottom: 16, boxShadow: '0 4px 20px rgba(27,43,94,0.08)', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', justifyContent: 'center' }}>
          ...
          <span style={{ fontSize: 12, color: totalColor, fontWeight: 600 }}>
        ...
        {total >= POINTS_MINIMUM && (
          <div style={{ marginTop: 12, padding: '8px 12px', background: 'rgba(245,161,36,0.08)', border: '1px solid rgba(245,161,36,0.3)', borderRadius: 8 }}>
            <div style={{ fontSize: 11.5, color: '#0d1632', lineHeight: 1.5 }}>
        ...
        {total > 0 && total < POINTS_MINIMUM && (
          <div style={{ marginTop: 12, padding: '8px 12px', background: 'rgba(220,38,38,0.08)', border: '1px solid rgba(220,38,38,0.35)', borderRadius: 8 }}>
            <div style={{ fontSize: 11.5, color: '#dc2626', lineHeight: 1.5 }}>
      ...
      {breakdown.length > 0 && (
        <div style={{ background: '#fff', border: `1.5px solid ${BORDER}`, borderRadius: 14, padding: '18px 16px', marginBottom: 12 }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: '#9ca3af', marginBottom: 12 }}>Points breakdown</div>
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 8 }}>
            {breakdown.map(r => (
              <div key={r.cat.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
                <div style={{ display: 'flex', gap: 6, alignItems: 'center', minWidth: 0 }}>
                  ...
                  <span style={{ fontSize: 12, color: '#374151', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.cat.heading}</span>
                </div>
                <span style={{ fontSize: 12, fontWeight: 700, color: r.cat.color, flexShrink: 0 }}>+{r.points}</span>
              </div>
            ))}
            <div style={{ borderTop: `1px solid ${BORDER}`, marginTop: 4, paddingTop: 8, display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ fontSize: 13, fontWeight: 800, color: totalColor }}>{total} pts</span>
            </div>
          </div>
        </div>
      )}

      <p style={{ fontSize: 11, color: '#9ca3af', margin: '0 0 12px', lineHeight: 1.6, textAlign: 'center' }}>
        ...
        <div style={{ background: '#fff', border: `1.5px solid ${BORDER}`, borderRadius: 14, padding: '16px 14px', marginBottom: 12 }}>
          {!showEmailGate ? (
            ...
            <form onSubmit={emailScore}>
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: '#9ca3af', marginBottom: 10 }}>Email my score</div>
              <input type="text" placeholder="Name (optional)" value={name} onChange={e => setName(e.target.value)}
                style={{ width: '100%', padding: '8px 10px', border: `1.5px solid ${BORDER}`, borderRadius: 8, fontSize: 13, marginBottom: 8, boxSizing: 'border-box' as const }} />
      ...
      {success && (
        <div style={{ background: 'rgba(245,161,36,0.12)', border: '1px solid rgba(245,161,36,0.35)', borderRadius: 10, padding: '10px 12px', marginBottom: 12, fontSize: 12, color: NAVY, textAlign: 'center' }}>
      ...
      <p style={{ fontSize: 10, color: '#c4c9d4', margin: '8px 0 0', textAlign: 'center' }}>
  ...
  return (
    <div id="points-calculator">
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 24 }}>
          ...
          <React.Fragment key={i}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{
                ...
                fontSize: 12, fontWeight: 700, color: i <= step ? '#fff' : '#9ca3af',
                flexShrink: 0,
              ...
              <span style={{ fontSize: 12, fontWeight: i === step ? 700 : 400, color: i === step ? NAVY : '#9ca3af', display: isMobile && i !== step ? 'none' : 'inline' }}>
                {label}
              </span>
            </div>
        ...
        <span style={{ fontSize: 12, color: '#9ca3af', marginLeft: 8, flexShrink: 0 }}>Step {step + 1} of 3</span>
      </div>
