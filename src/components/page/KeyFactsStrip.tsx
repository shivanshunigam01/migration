...
export function KeyFactsStrip({ facts, accent = NAVY, background = '#fff' }: KeyFactsStripProps) {
  return (
    <section style={{ background, borderBottom: `1px solid ${BORDER}` }}>
      <div className="key-facts-grid" style={{ maxWidth: 1200, margin: '0 auto', padding: '0 32px', display: 'grid', gridTemplateColumns: `repeat(${facts.length}, 1fr)` }}>
        {facts.map((fact, i) => (
          <article
            key={i}
          ...
          >
            {fact.icon ? (
              <>
                ...
                <div style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 20, fontWeight: 700, color: NAVY, lineHeight: 1.2, marginBottom: 4 }}>{fact.value}</div>
                <div style={{ fontSize: 13, fontWeight: 600, color: NAVY, marginBottom: fact.note ? 6 : 0 }}>{fact.label}</div>
                {fact.note && <div style={{ fontSize: 12, color: '#9ca3af', lineHeight: 1.5 }}>{fact.note}</div>}
              </>
            ) : (
              <>
                <div style={{ fontFamily: 'Fraunces, Georgia, serif', fontSize: 38, fontWeight: 700, color: accent, lineHeight: 1, marginBottom: 6 }}>{fact.value}</div>
                <div style={{ fontSize: 13, color: '#6b7280', lineHeight: 1.4 }}>{fact.label}</div>
              </>
            )}
          </article>
        ))}
