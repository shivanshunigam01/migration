    ...
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 20, alignItems: 'center' }}>
        ...
        <div style={{ position: 'relative', flex: '1 1 220px', minWidth: 200 }}>
          ...
          <input
            ...
            style={{ width: '100%', padding: '9px 12px 9px 34px', border: `1.5px solid ${BORDER}`, borderRadius: 8, fontSize: 14, color: NAVY, fontFamily: "'Gilroy', sans-serif", outline: 'none', boxSizing: 'border-box' }}
          />
        </div>
        ...
        {/* Result count */}
        <div style={{ fontSize: 13, color: '#6b7280', marginLeft: 'auto', whiteSpace: 'nowrap' }}>
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign: 'center', padding: '60px 24px', background: GREY_BG, borderRadius: 12, border: `1.5px dashed ${BORDER}` }}>
          ...
          <div style={{ fontSize: 17, fontWeight: 600, color: NAVY, marginBottom: 6 }}>No occupations match your search</div>
          <div style={{ fontSize: 14, color: '#9ca3af' }}>Try different keywords, or clear the filters to see all occupations.</div>
          <button onClick={() => { setSearch(''); setListFilter('All'); if (!lockVisaFilter) setVisaFilter('All') }}
            style={{ marginTop: 16, padding: '8px 18px', background: accent, color: '#fff', border: 'none', borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: "'Gilroy', sans-serif" }}>
            Clear filters
          </button>
        </div>
      )}
          ...
          {filtered.map(o => (
            <div key={o.anzscoCode} style={{ background: '#fff', border: `1.5px solid ${BORDER}`, borderRadius: 12, padding: '16px 18px' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 8, marginBottom: 8 }}>
                <div style={{ fontSize: 15, fontWeight: 600, color: NAVY, lineHeight: 1.4 }}>{o.title}</div>
                <ListBadge list={o.list} />
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 12px', marginBottom: 8 }}>
                <span style={{ fontSize: 12, color: '#6b7280' }}><strong>ANZSCO</strong> {o.anzscoCode}</span>
                <span style={{ fontSize: 12, color: '#6b7280' }}><strong>Assessing body</strong> {o.assessingAuthority}</span>
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, marginBottom: o.caveats ? 8 : 0 }}>
                {o.visas.map(v => (
                  <span key={v} style={{ fontSize: 11, fontWeight: 700, padding: '2px 7px', borderRadius: 100, background: `${accent}12`, color: accent, border: `1px solid ${accent}30` }}>{v}</span>
                ))}
              </div>
              {o.caveats && (
                <div style={{ fontSize: 12, color: '#f5a124', background: 'rgba(245,161,36,0.12)', border: '1px solid rgba(245,161,36,0.3)', borderRadius: 6, padding: '5px 9px', lineHeight: 1.5 }}>
                  {o.caveats}
                </div>
              )}
            </div>
            ...
            <thead>
              <tr style={{ background: NAVY, position: 'sticky', top: 0, zIndex: 2 }}>
                {['ANZSCO', 'Occupation title', 'List', 'Assessing authority', 'Visas', 'Caveats'].map(h => (
                  <th key={h} style={{ padding: '12px 14px', textAlign: 'left', fontSize: 12, fontWeight: 700, letterSpacing: '0.06em', color: '#fff', whiteSpace: 'nowrap', textTransform: 'uppercase' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {filtered.map((o, i) => (
                ...
                >
                  <td style={{ padding: '11px 14px', fontFamily: 'monospace', fontSize: 13, color: '#6b7280', whiteSpace: 'nowrap' }}>{o.anzscoCode}</td>
                  <td style={{ padding: '11px 14px', fontWeight: 500, color: NAVY, lineHeight: 1.4 }}>{o.title}</td>
                  <td style={{ padding: '11px 14px' }}>
                    <ListBadge list={o.list} />
                  </td>
                  <td style={{ padding: '11px 14px', color: '#374151', fontSize: 13 }}>{o.assessingAuthority}</td>
                  <td style={{ padding: '11px 14px' }}>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                      {o.visas.map(v => (
                        <span key={v} style={{ fontSize: 11, fontWeight: 700, padding: '2px 6px', borderRadius: 100, background: `${accent}10`, color: accent, border: `1px solid ${accent}28` }}>{v}</span>
                      ))}
                    </div>
                  </td>
                  <td style={{ padding: '11px 14px' }}>
                    {o.caveats ? (
                      <span style={{ fontSize: 12, color: '#0d1632', background: 'rgba(245,161,36,0.12)', border: '1px solid rgba(245,161,36,0.3)', borderRadius: 6, padding: '3px 7px', display: 'inline-flex', alignItems: 'center', gap: 3, lineHeight: 1.5 }}>
      ...
      {!isMobile && filtered.some(o => o.caveats) && (
        <div style={{ marginTop: 16 }}>
          <div style={{ fontSize: 12, fontWeight: 700, color: '#0d1632', marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Occupation caveats</div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {filtered.filter(o => o.caveats).map(o => (
              <div key={o.anzscoCode} style={{ display: 'flex', gap: 10, alignItems: 'flex-start', fontSize: 13 }}>
...
function ListBadge({ list }: { list: OccupationList }) {
  return (
    <span style={{ fontSize: 11, fontWeight: 700, padding: '3px 8px', borderRadius: 100, background: `${LIST_COLORS[list]}14`, color: LIST_COLORS[list], border: `1px solid ${LIST_COLORS[list]}30`, whiteSpace: 'nowrap' }}>
      {list}
    </span>
  )
}
